...
.. note:: 
   ...
   Since the :cpp:class:`DistMatrix\<T,U,V>` class makes use of MPI for 
...
The :cpp:class:`DistMatrix\<T,U,V>` class is implemented in a manner which 
...
:cpp:class:`AbstractDistMatrix\<T>`, which contains every member function which
whose prototype does not depend upon the exact matrix distribution, and 
:cpp:class:`DistMatrix\<T,U,V>`, which contains implementations of member 
